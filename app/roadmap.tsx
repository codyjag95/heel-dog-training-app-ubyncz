...
import { generateRoadmap, getRoadmapProgress, TrainingRoadmap, RoadmapDay, RoadmapWeek } from '../data/roadmapGenerator';
import { colors, typography, spacing } from '../data/darkTheme';

const GOLD = '#D2AF26';
const STORAGE_KEY_ROADMAP = '@heel_roadmap_v1';
const STORAGE_KEY_ROADMAP_START = '@heel_roadmap_start_v1';
...
export default function RoadmapScreen() {
  ...
  const { userProfile, hasPremium, lessonProgress, isLessonComplete } = useApp();
  ...
  useEffect(() => {
    const loadRoadmap = async () => {
      ...
      try {
        // Check for saved roadmap
        const saved = await AsyncStorage.getItem(STORAGE_KEY_ROADMAP);
        if (saved) {
          const parsed = JSON.parse(saved);
          // Verify it's for the same dog/breed combo
          if (parsed.dogName === userProfile.dogName && parsed.breedId === userProfile.breedId) {
            setRoadmap(parsed);
            setLoading(false);
            return;
          }
