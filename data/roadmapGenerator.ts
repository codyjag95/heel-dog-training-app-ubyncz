...
export type TrainingRoadmap = {
  ...
  breedId: string;
  generatedAt: string;
  signature: string;         // Fingerprint of the quiz inputs (cache validity)
  totalLessons: number;
  totalWeeks: number;
  weeks: RoadmapWeek[];
};
...
// ============================================================================
// EXPERIENCE-BASED LESSON GATING
// Advanced dogs shouldn't be told to teach "sit." We hide absolute-beginner
// lessons from FOUNDATION categories based on the quiz experience level.
// Specialized categories (reactivity, cooperative care, etc.) are never
// filtered — a well-trained dog can still be reactive.
// ============================================================================

// Foundation categories whose beginner lessons are redundant for dogs
// already past the basics.
export const FOUNDATION_CATEGORY_IDS = ['everyday_obedience', 'puppy_foundations', 'recall'];

// Quiz experience (1 = none … 4 = well trained) → minimum lesson difficulty
// we surface from foundation categories.
export function getExperienceMinDifficulty(experience: number): number {
  if (experience >= 4) return 3; // well trained → refinement only
  if (experience === 3) return 2; // some structure → skip absolute-beginner
  return 1;                       // none / basic → no filtering
}

// Filter a category's lessons by experience. Only foundation categories are
// gated; everything else passes through untouched.
export function filterLessonsByExperience(
  categoryId: string,
  lessons: Lesson[],
  experience: number
): Lesson[] {
  if (!FOUNDATION_CATEGORY_IDS.includes(categoryId)) return lessons;
  const minDiff = getExperienceMinDifficulty(experience);
  if (minDiff <= 1) return lessons;
  return lessons.filter(l => (l.difficulty ?? 1) >= minDiff);
}

// ============================================================================
// DAY THEME LABELS
// A day's theme reflects its primary category, so a barking plan reads like a
// barking plan (not a generic "Foundation"). Falls back per-week when a
// category has no dedicated label.
// ============================================================================

const CATEGORY_THEMES: { [categoryId: string]: string } = {
  puppy_foundations: 'Puppy Foundations',
  everyday_obedience: 'Foundation',
  potty_training: 'Potty Focus',
  biting_nipping: 'Bite Inhibition',
  calm_focus: 'Calm Training',
  leash_walks: 'Leash Skills',
  recall: 'Recall Basics',
  barking_alert: 'Barking Control',
  reactive_dog: 'Reactivity Support',
  socialization: 'Confidence Building',
  cooperative_care: 'Handling Practice',
  mental_work: 'Brain Games',
  real_world_proofing: 'Real-World Practice',
  handler_skills: 'Handler Skills',
  tricks: 'Trick Training',
  service_dog: 'Public Access Prep',
};

function themeForCategory(categoryId: string | undefined, fallback: string): string {
  if (!categoryId) return fallback;
  return CATEGORY_THEMES[categoryId] || fallback;
}

// ============================================================================
// PROFILE SIGNATURE
// A stable fingerprint of the quiz inputs that actually shape the plan. The
// roadmap cache is only reused when this matches, so retaking the quiz (e.g.
// changing the main issue) forces a fresh plan.
// ============================================================================
...
export function getProfileSignature(profile: {
  ...
  return [
    ...
    `rec:${(profile.recommendedCategories ?? []).join(',')}`,
    `ch:${[...(profile.challenges ?? [])].sort().join(',')}`,
    `q9:${[...(profile.q9Challenges ?? [])].sort().join(',')}`,
    `goal:${profile.q7Goal ?? ''}`,
    `age:${profile.ageStage || profile.ageLabel || ''}`,
  ].join('|');
}
...
function buildLessonPool(
  ...
  for (const catId of recommendedCategories) {
    ...
    if (category && category.lessons.length > 0) {
      const lessons = filterLessonsByExperience(catId, category.lessons, experience);
      if (lessons.length > 0) {
        pool.push({
          categoryId: catId,
          category,
          lessons: [...lessons], // Clone to avoid mutation
        });
      }
    }
  }
  ...
  for (const category of allCategories) {
    if (!recommendedCategories.includes(category.id)) {
      const lessons = filterLessonsByExperience(category.id, category.lessons, experience);
      if (lessons.length > 0) {
        pool.push({
...
export function generateRoadmap(profile: {
  ...
  const pool = buildLessonPool(recommendedCategories, CATEGORIES, experience);
  ...
  for (let d = 0; d < 7; d++) {
    ...
    // Fallback: if this day's categories are exhausted (e.g. foundation
    // lessons filtered out for an experienced dog), pull from any category so
    // the day is never empty.
    if (dayLessons.length === 0) {
      for (const entry of pool) {
        const fb = pickNextLesson(pool, entry.categoryId, usedLessonIds);
        if (fb) { dayLessons.push(fb); break; }
      }
    }

    // Determine day theme based on the day's primary category, so the plan
    // reads like the user's actual concern (e.g. "Barking Control").
    const theme = themeForCategory(dayLessons[0]?.categoryId || primaryCat, 'Foundation');
  ...
  for (let d = 0; d < 7; d++) {
    ...
    // Fallback so the day is never empty (see Week 1).
    if (dayLessons.length === 0) {
      for (const entry of pool) {
        const fb = pickNextLesson(pool, entry.categoryId, usedLessonIds);
        if (fb) { dayLessons.push(fb); break; }
    ...
    const theme = themeForCategory(dayLessons[0]?.categoryId || primaryCat, 'Building Habits');
  ...
  for (let d = 0; d < 7; d++) {
    ...
    week3Days.push({
      ...
      dayOfWeek: DAY_NAMES[d],
      isRestDay: false,
      theme: themeForCategory(dayLessons[0]?.categoryId, d < 3 ? 'Real-World Practice' : 'Handler Growth'),
      lessons: dayLessons,
    });
  }
  ...
  for (let d = 0; d < 7; d++) {
    ...
    const placedCat = dayLessons[0]?.categoryId || primaryCat;
    let theme = themeForCategory(placedCat, 'Advanced Skills');
    if (placedCat === 'tricks') theme = 'Show-Off Tricks'; // week-4 flavor
  ...
  return {
    ...
    signature: getProfileSignature(profile),
    totalLessons,
    totalWeeks: 4,
    weeks,
  };
