...
export function buildProfileFromAnswers(answers: any[], dogName: string, allAnswers?: any[]) {
  ...
  const profile = {
    ...
    q7Goal: '' as string,
    ageLabel: '' as string,
    ageStage: '' as string,   // NEW: 'puppy' | 'adolescent' | 'adult' | 'senior' (from Q3)
  ...
  answersToProcess.forEach(answer => {
    ...
    if (questionId === 'q3') {
      ...
      if (ageAnswer) {
        ...
        // Derive a stable age stage for recommendation logic (breed field untouched)
        if (answerId === 'q3_a1') profile.ageStage = 'puppy';
        else if (answerId === 'q3_a2') profile.ageStage = 'adolescent';
        else if (answerId === 'q3_a3') profile.ageStage = 'adult';
        else if (answerId === 'q3_a4') profile.ageStage = 'senior';
...
// Categories that are actually premium (isPremium: true in categoryData.ts).
// Used only by the recommendation engine's "premium guarantee" below — NOT by
// lesson gating (that lives in utils/premiumAccess.ts). 'tricks' was removed
// here because it is a FREE category in categoryData.ts; keeping it in this
// list let a free category satisfy the premium guarantee, which defeated it.
...
function determineRecommendedCategories(profile: any): string[] {
  ...
  // TIER 0.5: PUPPY / ADOLESCENT FOUNDATIONS
  // Young dogs get the puppy track early — but never above a real Q9
  // emergency, which is why this sits just below the urgent tier.
  if (profile.ageStage === 'puppy' || profile.ageStage === 'adolescent') {
    addIfNew(recommended, 'puppy_foundations');
  }
  ...
  if (challenges.includes('barking')) {
    // Barking is its own dedicated category. Surface it first (primary
    // concern), with Calm & Focus as supporting work.
    addIfNew(recommended, 'barking_alert');
    addIfNew(recommended, 'calm_focus');
  }
  ...
  if (q7Goal) {
    switch (q7Goal) {
      ...
      case 'goal_focus':
        // "Better focus & listening" spans obedience (listening) and impulse
        // control (focus), so surface both.
        addIfNew(recommended, 'everyday_obedience');
        addIfNew(recommended, 'calm_focus');
        break;
  ...
  if (!hasPremiumCategory) {
    // Pick a genuinely-premium category so the guarantee actually holds.
    let premiumPick: string;
    if (experience >= 3) premiumPick = 'handler_skills';
    else if (energyLevel >= 7) premiumPick = 'real_world_proofing';
    else premiumPick = 'cooperative_care';
